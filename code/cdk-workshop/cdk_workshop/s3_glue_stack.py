...
class S3GlueStack(cdk.Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        glue_crawler = glue.CfnCrawler(self, id='crawler_cdk_workshop',
        ...
        lambda_func = _lambda.Function(self, 'LambdaTrigger',
                                       ...
                                       code=_lambda.Code.from_asset('lambda_funcs'), # ToDo: Fehler einbauen
                                       environment={'GLUE_CRAWLER': glue_crawler.name})

        lambda_func.role.add_managed_policy(iam.ManagedPolicy.from_aws_managed_policy_name("service-role/AWSGlueServiceRole")) # ToDo: Fehler einbauen

        # S3 Event Trigger
        lambda_trigger = _lambda_event.S3EventSource(s3_landingzone, events= [
                s3.EventType.OBJECT_CREATED_PUT,
                s3.EventType.OBJECT_CREATED_COMPLETE_MULTIPART_UPLOAD   # ToDo: Fehler einbauen
            ])
        lambda_func.add_event_source(lambda_trigger)
        # notification = s3n.LambdaDestination(lambda_func)  # https://aws.amazon.com/de/blogs/big-data/run-aws-glue-crawlers-using-amazon-s3-event-notifications/
        # notification.bind(self, s3_landingzone)
